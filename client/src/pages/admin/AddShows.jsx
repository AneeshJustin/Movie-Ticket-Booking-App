    ...
    return !loading ? (
        <>
                    ...
                    {nowPlayingMovies.map((movie) => (
                        <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`} onClick={() => setSelectedMovie(movie.id)}>
                            <div className='relative rounded-lg overflow-hidden'>
                                <img
                                    src={movie.poster_path ? `${image_base_url}/${movie.poster_path.replace(/^\/+/, '')}` : 'https://placehold.co/500x750?text=No+Poster'}
                                    onError={(e) => { e.target.src = 'https://placehold.co/500x750?text=Error+Loading+Poster'; e.target.onerror = null; }}
                                    alt="" className='w-full object-cover brightness-90'
                                />
            ...
            {Object.keys(dateTimeSelection).length > 0 && (
                <div className='mt-6'>
                    <h2 className='mb-2'>Selected Dates - Times</h2>
