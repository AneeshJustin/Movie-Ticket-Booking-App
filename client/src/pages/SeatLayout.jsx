...
const SeatLayout = () => {
    ...
    const handleSeatClick = (seatId) => {
        ...
        if (occupiedSeats.includes(seatId)) { return toast("Seat is already booked") }
    ...
    const bookTickets = async () => {
        try {
            if (!user) return toast.error("Please login to book a ticket")
        ...
        <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50'>
            ...
            <div className='w-60 bg-primary/10 border border-primary/20 rounded-lg py-10 h-max md:sticky md:top-30'>
                ...
                <div className='mt-5 space-y-1'>
                    {show.dateTime[date].map((item) => (<div key={item.time} onClick={() => setSelectedTime(item)} className={`flex items-center gap-2 px-6 py-2 w-max rounded-r-md cursor-pointer transition ${selectedTime?.time === item.time ? 'bg-primary text-white' : 'hover:bg-primary/20'}`}>
            ...
            <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
                ...
                <div className='flex flex-col items-center mt-10 text-xs text-gray-300'>
                    ...
                    <div className='grid grid-cols-2 gap-11'>
                        {groupRows.slice(1).map((group, idx) => (<div key={idx}>
